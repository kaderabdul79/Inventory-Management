<template>
    <div class="updatebrand">{{ form }}
        <v-container fluid>
            <v-row>
                <v-col cols="6" xs="12" offset="3">
                    <v-sheet>
                        <v-sheet class="text-h4 py-2 text-center">Edit Brand</v-sheet>
...
const form = ref(new Form(
    ...
    function getbrand(){
        axios.get('brands/'+props.id+'/edit')
        .then(response => {
    ...
    function updateBrand(){
        axios.put('brands/'+props.id, {
            name: form.value.name,
            is_active: form.value.is_active
        })
            .then(response => {
                // console.error(response);
                notification.notify({title: "brand Updated 🎉",});
                setTimeout(()=>{
