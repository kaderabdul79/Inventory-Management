<template>
    <div class="updatesize">{{ form }}
        <v-container fluid>
            <v-row>
                <v-col cols="6" xs="12" offset="3">
                    <v-sheet>
                        <v-sheet class="text-h4 py-2 text-center">Edit Size</v-sheet>
                        <v-form @submit.prevent="updateSize">
...
const form = ref(new Form(
    ...
    function updateSize(){
        axios.put('sizes/'+props.id, {
            name: form.value.name,
            is_active: form.value.is_active
        })
            .then(response => {
                notification.notify({title: "size Updated 🎉",});
                setTimeout(()=>{
